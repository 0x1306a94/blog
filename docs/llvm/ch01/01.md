...
struct FunctionCallTimePass : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
        ...
        // 可以通过 __attribute__((__annotate__(("ignore_appletrace")))) 忽略当前函数
        if (annotation.length() > 0 && StringRef(annotation).contains("ignore_appletrace")) {
            errs() << "function-call-time: " << funcName << " 忽略 annotation"
                   << "\n";
            return false;
        }
        // 将统计代码调用过滤掉
        if (funcName.contains("appletrace")) {
            return false;
        }
        // 如果是插桩的函数直接跳过
    ...
    void insertEndInst(Function &F) {
        ...
        for (Function::iterator I = F.begin(), E = F.end(); I != E; ++I) {
            ...
            for (BasicBlock::iterator I = BB.begin(), E = BB.end(); I != E; ++I) {
                ...
                errs() << "function-call-time: " << BB.getParent()->getName() << " end\n";
            }
        }
    }
};
...
static RegisterPass<FunctionCallTimePass>
    X("function-call-time", "Function calls take time to collect", false, false);
...
```shell
LLVM/clang-11.0.0/bin/clang -x objective-c++ -isysroot $(xcrun --sdk macosx --show-sdk-path) -emit-llvm -c xxx.m -o xx.bc
```
...
- [Clang插件统计方法耗时](http://www.helloted.com/ios/2020/09/22/clang-time/)
